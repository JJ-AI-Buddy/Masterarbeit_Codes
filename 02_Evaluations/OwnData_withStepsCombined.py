...
#############################################
##### SET INPUTS    
bool_trans = True    # if True then bool_rot has to be False
...
ID = 'C601'        # Set ID like specified in the documentation

# ICP parameters
max_correspondence_distance = 0.3   # max. distance between two points to be seen as correct correspondence (=: inlier)
...
number_eval_points = [17,17,17,17,17,17]   #[17,17,17,17,17,17] for 1D, [9,9,9,9,9,9] for 2D, [21,21,1,10,10,10] for big offsets (-50,50; -np.pi/2,np.pi/2)
...
name_txt = str(ID) + '_Transl-ICP_01TRC.txt'
path_txt = os.path.join(path_to_file, name_txt)
name_csv = str(ID) + '_Transl-ICP_01TRC.csv'
path_csv = os.path.join(path_to_file,name_csv)
name_csv_iter = str(ID) + '_TranslIter-ICP_01TRC.csv'
...
with open(path_txt, 'w') as f:
    f.write('Evaluation of ICP algorithm for map matching on own dataset ~01_TRC-Skidpad~ \n' + str(datetime.now()) + "\n\n")
...
# Choose the GT of the timestamp to be evaluated - on own dataset not necessary!!!
#sample_step = 100
...
#Load GNSS poses from csv   - on own dataset not necessary!!!1
...
#Point Clouds - paths to the Point Clouds are also saved in first column of the GT csv.-file
    ...
    if bool_1D == True:
        ...
        for k in range(k0, k0+3):
            ...
            for n in eval_points:
                ...
                print(
                    "ICP Registration for variation in direction %f; value of delta = %f" % (k, n))
