...
main()
{
    ...
    for (const auto& vec : listOfExperiments) {
        ...
        float lower_limits[6] = { -2,-2,-2,-M_PI / 4,-M_PI / 4, -M_PI / 4 };//{ -50, -50, 0, -M_PI / 2, -M_PI / 2, -M_PI / 2 }; //{ -2,-2,-2,-M_PI / 4,-M_PI / 4, -M_PI / 4 };
        float upper_limits[6] = { 2,2,2,M_PI / 4, M_PI / 4, M_PI / 4 }; //{ 50,50,0.01, M_PI / 2,M_PI / 2,M_PI / 2 }; //{ 2,2,2,M_PI / 4, M_PI / 4, M_PI / 4 };

        int number_eval_points[6] = { 17,17,17,17,17,17 }; //{ 21,21,2,10,10,10 };//{ 17,17,17,17,17,17 };    // 6 axes
        ...
        string path_map = "C:\\Users\\Johanna\\OneDrive - bwedu\\Masterarbeit_OSU\\Evaluations\\01_TRC_Skidpad_Data\\01_Map_GPS_TRC_1234.pcd";
        string path_GT_csv = "C:\\Users\\Johanna\\OneDrive - bwedu\\Masterarbeit_OSU\\Evaluations\\01_TRC_Skidpad_Data\\01_TRC-Skidpad_GT_Poses_Run5.csv";
        //string path_GNSS_csv = "C:\\Users\\Johanna\\OneDrive - bwedu\\Masterarbeit_OSU\\Baseline\\02_Moriyama_Data\\13_GNSS_pose.csv";
        string path_to_file = "C:\\Users\\Johanna\\OneDrive - bwedu\\Masterarbeit_OSU\\Evaluations\\01_TRC_Skidpad_Evaluation";

        string name_txt = '\\' + ID + type + "EvalNDTOwnData.txt";
        string path_txt;
        path_txt = path_to_file + name_txt;
        string name_csv = '\\' + ID + type + "EvalNDTOwnData.csv";
        string path_csv;
        path_csv = path_to_file + name_csv;
        string name_csv_iter = '\\' + ID + "_IterStepsEvalNDTOwnData.csv";
        ...
        outfile_text << "Evaluation of NDT algorithm for map matching on own dataset \n" << std::endl;
        ...
        //Sampling of the timestamps not necessary
        //int i = 3;
        //int sample_step = 100;
        ...
        // No GNSS poses necessary because GNSS is already GT
        ...
        if (pcl::io::loadPCDFile<pcl::PointXYZ>(pc_scan_path[ind], *input_cloud) == -1)
        {
        ...
        Eigen::Quaternionf quaternion_GT(col_theta[ind], col_alpha[ind], col_beta[ind], col_gamma[ind]);
        Eigen::Matrix3f init_rotation_GT = quaternion_GT.normalized().toRotationMatrix();
        Eigen::Vector4f init_translation_GT; init_translation_GT << col_x[ind], col_y[ind], col_z[ind], 1.0;
        ...
        //Calculate GNSS transformation - not necessary here!!!!
        ...
        float rse_transl_GNSS = 0.0;
        float rso_deg_1_GNSS = 0.0;
        float rso_deg_2_GNSS = 0.0;
        ...
        // Write data to the file
        outfile_text << "\n\n" << "-------------------------------------------" << std::endl;
        outfile_text << "\n\nSource point cloud:\nTimestamp = " + to_string(col_time[ind]) << std::endl;
        ...
        if (bool_1D == true) {
            ...
            for (int k = 0; k < 3; k++) {
                ...
                for (int n = 0; n < number_eval_points[counter]; n++) {
                    ...
                    // Write data to the file
                    outfile_csv << ID << ";" << col_time[ind] << ";" << k << ";" << output_offset[0] << ";" << output_offset[1] << ";" << output_offset[2] << ";"
                    ...
                    // Write data to the file
                    outfile_csv_iter << ID << ";" << col_time[ind] << ";" << k << ";" << eval_points[n] << ";" << number_iterations << ";" << fitness << ";"
        ...
        if (bool_2D == true) {
            ...
            for (int k = 0; k < number_eval_points[axis2Deval[0]]; k++) {
                ...
                for (int n = 0; n < number_eval_points[axis2Deval[1]]; n++) {
                    ...
                    // Write data to the file
                    outfile_csv << ID << ";" << col_time[ind] << ";" << axis2Deval[0] << "," << axis2Deval[1] << ";" << list_axes[0] << ";" << list_axes[1] << ";" << list_axes[2] << ";"
                        << rse_transl_init << ";" << rso_deg_1_init << ";" << rso_deg_2_init << ";" << fitness << ";"
                        << outlier_ratio << ";" << rse_transl << ";" << rso_deg_1 << ";" << rso_deg_2 << ";"
                        << number_iterations << ";" << (duration.count() / 1000) << ";" << rse_transl_GNSS << ";" << rso_deg_1_GNSS << ";" << rso_deg_2_GNSS
                        << std::endl;
                    ...
                    // Write data to the file
                    outfile_csv_iter << ID << ";" << col_time[ind] << ";" << axis2Deval[0] << "," << axis2Deval[1]
            ...
            for (int k = 0; k < number_eval_points[axis2Deval[0]]; k++) {
                ...
                for (int n = 0; n < number_eval_points[axis2Deval[1]]; n++) {
                    ...
                    for (int w = 0; w < number_eval_points[axis2Deval[2]]; w++) {
                        ...
                        // Write data to the file
                        outfile_csv << ID << ";" << col_time[ind] << ";" << axis2Deval[0] << "," << axis2Deval[1] << "," << axis2Deval[2]
                        ...
                        // Write data to the file
                        outfile_csv_iter << ID << ";" << col_time[ind] << ";" << axis2Deval[0] << "," << axis2Deval[1] << "," << axis2Deval[2] << ";"
                            << rse_transl_init << ";" << number_iterations << ";" << fitness << ";"
                            << final_transform(0, 0) << ";" << final_transform(0, 1) << ";" << final_transform(0, 2) << ";" << final_transform(0, 3) << ";"
                            << final_transform(1, 0) << ";" << final_transform(1, 1) << ";" << final_transform(1, 2) << ";" << final_transform(1, 3) << ";"
                            << final_transform(2, 0) << ";" << final_transform(2, 1) << ";" << final_transform(2, 2) << ";" << final_transform(2, 3) << ";"
