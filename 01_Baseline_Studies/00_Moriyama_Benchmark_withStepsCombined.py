...
Notes:
    - This script provides an implementation of the Point-to-Point and Point-to_Plane ICP registration algorithm with the Open3D library
    - Goal: Evaluate the influence of initial pose perturbations on the performance of point cloud registration using the open-source 'Moriyama' dataset from Autoware
    - "withSteps" means that this special implementation yields the results and evaluation metric after every iteration step and not only the final result
    - "Combined" means that not only 1D translatory or rotatory perturbation (x,y or z; roll, pitch or yaw) of the intial pose possible; also 2D perturbations (x and y) and 3D perturbations (x,y and yaw) are 

    - !!!! It is recommened to use Spyder as IDE because changes have to be made to the script before every execution !!!
"""
...
############################################# Has to be edited before every execution
##### SET INPUTS ############################ 
