...
#vis.destroy_window()
    ...
    #### ICP ###############################################################
    threshold = 0.1     # max. distance between two points to be seen as correct correspondence (=: inlier)
    trans_init = transform_GT.copy()

    #steps_init = [0.5, 0.5,0.5]   #step size in x,y,z direction
    lower_limits = [-np.pi/2,-np.pi/2,-np.pi/2]    #x,y,z
    upper_limits = [np.pi/2,np.pi/2,np.pi/2]       #x,y,z
